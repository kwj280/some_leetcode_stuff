...
@Throws(NoSupportedLanguageException::class)
fun parseAcceptLanguage(clientLanguage: String, supportedLanguages: Array<String>): Array<String> {
    if (clientLanguage.isEmpty() || supportedLanguages.isEmpty()) {
        throw NoSupportedLanguageException("No supported language found!")
    }
    val supportedLanguageSet = supportedLanguages.toSet()
    val parsedLanguages = mutableListOf<String>()

    clientLanguage.split(",").sorted().forEach { language ->
        val trimmed = language.trim()
        if (supportedLanguageSet.contains(trimmed)) {
            parsedLanguages.add(trimmed)
        }
    }
    parsedLanguages.sort()

    return parsedLanguages.toTypedArray()
}


class NoSupportedLanguageException : Exception {
    constructor() : super()
